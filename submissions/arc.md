...
`GuardVaultCCTP` (Base Sepolia `0xbEa47f1B7C1252EB2a7C758b27f28ea35ae6c193`) calls the real CCTP V2 `ITokenMessengerV2.depositForBurn` with `destinationDomain = 26` (Arc); its `rescue()` is Flex-mandate-bounded. **Round-trip verified end-to-end:** the Base burn (`0x285c2773761185853fcd5113cb62a6a804a939fcae34246cd79267ee4da890ce`) was **minted on Arc** — re-submitting the same CCTP message to Arc's MessageTransmitterV2 (`0xE737e5cE…`) reverts with `"Nonce already used"`, which is on-chain proof the destination consumed/minted the message. (Arc testnet prunes historical logs, so the exact Arc mint tx is read from arcscan's view of the recipient's incoming CCTP rather than a log query.)
...
4. **CCTP cross-chain rescue:** show the burn on Base (`GuardVaultCCTP` `0xbEa47f…`, `depositForBurn`, `destinationDomain 26`) on basescan, then state it's a **verified round-trip** — the message was minted on Arc (re-submitting the same message reverts "Nonce already used"). Real USDC crossed Base → Arc over CCTP V2.

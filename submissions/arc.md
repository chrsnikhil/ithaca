...
`GuardVaultCCTP` (Base Sepolia `0xbEa47f1B7C1252EB2a7C758b27f28ea35ae6c193`) calls the real CCTP V2 `ITokenMessengerV2.depositForBurn` with `destinationDomain = 26` (Arc); its `rescue()` is Flex-mandate-bounded. **Honest scope:** the **burn side is proven on Base** and the **attestation + mint are implemented** — a full round-trip Arc **mint** transaction is not yet captured. We say "burn proven on Base; attestation + mint implemented," not "full round-trip proven."
...
4. **CCTP cross-chain rescue:** trigger the burn on Base (`GuardVaultCCTP` `0xbEa47f…`, `depositForBurn`, `destinationDomain 26`) and show it on basescan — be explicit: "burn proven here; attestation and mint are implemented, full round-trip mint not yet captured."

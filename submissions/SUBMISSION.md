...
- **CCTP:** round-trip **verified end-to-end** — the Base burn (`0x285c2773…`) was minted on Arc; re-submitting the same CCTP message to Arc's MessageTransmitterV2 reverts `"Nonce already used"`, proving the destination consumed it. Arc testnet prunes historical logs, so the exact Arc mint tx is read from arcscan rather than a log query.
...
- **Ledger Key Ring:** the wallet-cli ring (secret decrypt) and LKRP USB-less enrollment (software-signed `addMember`, enrolling a Ledger-less headless host) are **implemented against the real Ledger SDKs**; the **tested default** path is a local AES-GCM backend.

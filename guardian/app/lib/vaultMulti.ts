...
const RPC = process.env.BASE_SEPOLIA_RPC || B.readRpc; // publicnode — sepolia.base.org serves stale reads
const EXPLORER = B.explorerBase;
const CHAIN_ID = B.chainId;
const VAULT = B.GuardianVaultMulti;
const DEFAULT_TOKEN = B.aaveUsdc; // the token the multi vault actually holds (read via usdc() when possible)
...
export const MARKETS = B.multiMarkets; // [{ id, name, venue, apr }]
export const venueOf = (id: string) => MARKETS.find((m) => m.id === id)?.venue ?? null;
export const marketByVenue = (v?: string) =>
...
export function provider() { return new ethers.JsonRpcProvider(RPC); }
...
export function verifyPolicy(policy: Policy, signature: string): { ok: boolean; signer?: string; error?: string } {
  try {
...
export function verifyEscalation(e: Escalation, signature: string): { ok: boolean; signer?: string; error?: string } {
  try {
...
// --- reads ---
export async function readMultiState(haven?: string): Promise<MultiState> {
  const v = readVault();
...
// even with no daemon.
export async function readMultiFeed() {
  try {
    const p = provider();
